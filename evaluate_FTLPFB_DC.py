...
if __name__ == '__main__':
    ...
    base_means = []
    base_cov = []
    base_features_path = "./checkpoints/%s/%s_S2M2_R/last/base_features.plk"%(dataset, model)
    ...
    F_path = './checkpoints/%s/%s_S2M2_R/%d.tar'%(dataset, model, epoch)
    H_path = './checkpoints/%s/%s_S2M2_R/linear_layer.pth'%(dataset, model)
    F_model = load_model(F_model, F_path, dataset)
    for firth_c in [1]:# [0.0, 0.1, 0.3, 1.0, 3.0, 10.0, 30.0, 100.0]:
        acc_list = []
        ...
        for i, (ndatas, labels) in tqdm(enumerate(dataloader)):
            ...
            '''
            support_features, _ = finetuning.F_model(support_data)
            query_features, _ = finetuning.F_model(query_data)

...
if __name__ == '__main__':
    ...
    for firth_c in [0.1, 0.3, 1.0, 3.0, 10.0, 30.0, 100.0]:
        acc_list = []
        ...
        for i, (ndatas, labels) in tqdm(enumerate(dataloader)):
            ...
            scores = linear_classifier(X_aug, Y_aug, query_features, firth_c = True)
            scores = scores.detach().cpu().numpy()
            predicts_wo_finetune = np.argmax(scores, axis = -1)
        
            H_model = linear_classifier.linear_clf
            finetuning = FineTuning(F_model, H_model, F_path, H_path)
            finetuning.train_loop(support_data, torch.tensor(support_label))
